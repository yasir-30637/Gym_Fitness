...
class _WeightGainState extends State<WeightGain> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text('Weight Gain Diet'),
      ),
      body: Padding(
              ...
              children: [
                  ...
                  onPressed: isAdmin ? ()async {
                    try {
                      ...
                      if (name.isNotEmpty && contact.isNotEmpty) {
                        String id = FirebaseFirestore.instance.collection('weightGain').doc().id;
                        await FirebaseFirestore.instance.collection('weightGain').doc('$name' + '' + '$contact' + '' + '$id').set({
                                'name': name,
                            'contact': contact,
                                   'id': id,
                        }).then((value) {
                ...
                ElevatedButton(
                    onPressed: () async{
                      ...
                      try {
                        QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('weightGain').get();
              ...
              InkWell(
                onTap:isAdmin ? () async{
                  try {
                    ...
                    await FirebaseFirestore.instance.collection('weightGain').doc(id).delete().then((value) {
                      setState(() {
